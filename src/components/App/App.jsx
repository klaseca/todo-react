...
export default class App extends Component {
  ...
  saveMarkerValue = (id, value) => {
    this.setState(({ markers }) => {
      const newMarkers = [...markers];
      const markerIndex = markers.findIndex(elem => {
        return elem.id === id;
      });

      newMarkers[markerIndex].value = value;
	...
	removeMarker = (id) => {
		this.setState(({ markers, todos, trash }) => {
      const newMarkers = markers.filter(elem => {
        return elem.id !== id;
			});
			
			const newTodos = todos.map(elem => {
				const markers = elem.markers.filter(idMarker => {
					return idMarker !== id;
				});
      ...
      return { markers: newMarkers, todos: newTodos, trash: newTrash };
    });
	}
  ...
  state = {
    ...
    setCheckedMarker: this.setCheckedMarker,
		saveMarkerValue: this.saveMarkerValue,
